...
(function () {
  ...
  function updateWarmth() {
    ...
    document.documentElement.style.setProperty('--bg', rgbToCss(currentColor));
  }
...
// Minimal draggable scrollbar overlay with no layout impact.
(function () {
  ...
  function getMetrics() {
    ...
    var thumbHeight = Math.max(32, (viewportHeight * viewportHeight) / documentHeight);
    return {
      viewportHeight: viewportHeight,
      maxScroll: Math.max(0, documentHeight - viewportHeight),
      thumbHeight: thumbHeight,
      maxThumbTravel: Math.max(0, viewportHeight - thumbHeight),
    };
  }

  function updateScrollbar() {
    var metrics = getMetrics();
    thumb.hidden = metrics.maxScroll === 0;
    if (thumb.hidden) return;

    var y = (window.scrollY / metrics.maxScroll) * metrics.maxThumbTravel;
  ...
  thumb.addEventListener('pointermove', function (event) {
    if (!dragging) return;
    var metrics = getMetrics();
    if (!metrics.maxThumbTravel) return;
    var scrollDelta = ((event.clientY - dragStartY) / metrics.maxThumbTravel) * metrics.maxScroll;
  ...
  document.addEventListener(
    'pointermove',
    function (event) {
      if (window.innerWidth - event.clientX <= 20) showScrollbar();
    },
    { passive: true }
  );
  document.addEventListener('htmx:afterSettle', updateScrollbar);
